 ...
 */
@Controller
public class PinglunhuifuController extends BaseController {

    @Autowired
    private PinglunhuifuMapper dao;

    @Autowired
    private PinglunhuifuService service;
     ...
     *
     */
    @RequestMapping("/pinglunhuifu_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        List<Pinglunhuifu> list = service.selectPage(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "pinglunhuifu_list"; // 使用视图文件：WebRoot\pinglunhuifu_list.jsp
    }
     ...
     * 评论人列表
     */
    @RequestMapping("/pinglunhuifu_list_pinglunren")
    public String listpinglunren() {
        ...
        List<Pinglunhuifu> list = service.selectPage(example, page, 12);

        request.setAttribute("list", list);
        assign("orderby", order);
        assign("sort", sort);
        assign("where", where);
        return "pinglunhuifu_list_pinglunren";
    }

    /**
     * 回复人列表
     */
    @RequestMapping("/pinglunhuifu_list_huifuren")
    public String listhuifuren() {
        ...
        assign("sort", sort);
        assign("where", where);
        return "pinglunhuifu_list_huifuren";
    }

    @RequestMapping("/pinglunhuifuadd")
    public String addWeb() {
        ...
        return "pinglunhuifuadd";
    }

    @RequestMapping("/pinglunhuifu_updt")
    public String updt() {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Pinglunhuifu mmm = service.find(id);
        request.setAttribute("mmm", mmm);
        request.setAttribute("updtself", 0);
        return "pinglunhuifu_updt";
    }
     ...
     * @return
     */
    @RequestMapping("/pinglunhuifuinsert")
    public String insert() {
        String tmp = "";
        Pinglunhuifu post = new Pinglunhuifu(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
     ...
     * @return
     */
    @RequestMapping("/pinglunhuifuupdate")
    public String update() {
        // 创建实体类
        Pinglunhuifu post = new Pinglunhuifu();
        // 将前台表单数据填充到实体类
     ...
     *  删除
     */
    @RequestMapping("/pinglunhuifu_delete")
    public String delete() {
        if (!checkLogin()) {
            return showError("尚未登录");
        }
        int id = Request.getInt("id"); // 根据id 删除某行数据
        Map map = Query.make("pinglunhuifu").find(id);
