 ...
 */
@Controller
public class KuaixunController extends BaseController {

    @Autowired
    private KuaixunMapper dao;

    @Autowired
    private KuaixunService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/kuaixun_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        List<Kuaixun> list = service.selectPage(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "kuaixun_list"; // 使用视图文件：WebRoot\kuaixun_list.jsp
    }
    ...
    public String index() {
        ...
        List<Kuaixun> list = service.selectPage(example, page, 12);
    ...
    @RequestMapping("/kuaixun_updt")
    public String updt() {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Kuaixun mmm = service.find(id);
     ...
     * @return
     */
    @RequestMapping("/kuaixuninsert")
    public String insert() {
        String tmp = "";
        Kuaixun post = new Kuaixun(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        post.setFabushijian(Info.getDateStr());
     ...
     * @return
     */
    @RequestMapping("/kuaixunupdate")
    public String update() {
        // 创建实体类
        Kuaixun post = new Kuaixun();
        // 将前台表单数据填充到实体类
        ...
        if (!Request.get("fabushijian").equals("")) post.setFabushijian(Request.get("fabushijian"));
     ...
     *  后台详情
     */
    @RequestMapping("/kuaixun_detail")
    public String detail() {
        if (!checkLogin()) {
            return showError("尚未登录", "./");
        }
        int id = Request.getInt("id");
        Kuaixun map = service.find(id); // 根据前台url 参数中的id获取行数据
        request.setAttribute("map", map); // 把数据写到前台
        return "kuaixun_detail"; // 详情页面：WebRoot\kuaixun_detail.jsp
    }

    /**
     *  前台详情
     */
    @RequestMapping("/kuaixundetail")
    public String detailweb() {
        int id = Request.getInt("id");
        Kuaixun map = service.find(id);

        request.setAttribute("map", map);
        return "kuaixundetail"; // 前台详情页面：WebRoot\kuaixundetail.jsp
    }

    /**
     *  删除
     */
    @RequestMapping("/kuaixun_delete")
    public String delete() {
        if (!checkLogin()) {
            return showError("尚未登录");
        }
        int id = Request.getInt("id"); // 根据id 删除某行数据
        Map map = Query.make("kuaixun").find(id);
